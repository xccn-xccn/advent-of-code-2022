...
def calculate(value, equation):
    ...
    return eval(f"{value} {operator} {number.group(0) if number else value}")


def single_monkey(notes, mod):
    yes, no = [], []
    for item in notes[0]:
        item = item % mod
...
def main():
    ...
    mod = reduce(lambda x, y: x*y, [m[2] for m in monkeys])
    for repeat in range(10_000):
        for notes in monkeys:
            notes[-1] += len(notes[0])
            yes, no = single_monkey(notes, mod)
